...
const WeatherSetting = ({ handleCurrentPageChange,handleCityChange,cityName }) => {
  
  //定義一個state來取得使用者選的地名
  const [locationName,setLocationName]=useState(cityName);

  //使用者選擇地名後透過OnChange()呼叫handleChange()改變react內的資料
  const handleChange=(e)=>{
    setLocationName(e.target.value);
  }
  //當使用者點擊SAVE後透過OnClick()呼叫handleSave()並將畫面轉回WeatherCard
  const handleSave=()=>{
